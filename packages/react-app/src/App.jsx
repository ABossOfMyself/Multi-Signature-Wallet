...
// let BACKEND_URL = "https://localhost:49832/";

let BACKEND_URL = "https://cyan-distinct-moose.cyclic.app/";

// 📡 What chain are your contracts deployed to?

const initialNetwork = NETWORKS.goerli; // <------- select your target frontend network (localhost, rinkeby, goerli, xdai, mainnet)
...
function App(props) {
  ...
  // MultiSigWalletCreator Events
  const ownersMultiSigEvents = useEventListener(readContracts, "MultiSigWalletCreator", "Owners", localProvider, 1);
  ...
  useEffect(()=>{
    if(userSigner){
      setCurrentMultiSigAddress("0x0000000000000000000000000000000000000000")
    }
  },[userSigner])
  */
  ...
  const signaturesRequiredContract = useContractReader(readContracts, contractName, "getSignaturesRequired");
  const nonceContract = useContractReader(readContracts, contractName, "getNonce");
  ...
  useEffect(() => {
    async function getContractValues() {
      const latestSignaturesRequired = await readContracts.MultiSigWallet.getSignaturesRequired();
      setSignaturesRequired(latestSignaturesRequired);
      
      const nonce = await readContracts.MultiSigWallet.getNonce();
      setNonce(nonce);
    }
  ...
  useEffect(() => {
    const filteredEvents = allExecuteTransactionEvents.filter(event => event.address === currentMultiSigAddress);
  ...
  */
  ...
  return (
    <div className="App">
      ...
      >
        <Menu.Item key="/">
          <Link to="/">Multi Signature Wallet</Link>
        </Menu.Item>
                ...
                <Alert
                  message={
                    <>
                      ...
                      or Select your Multi Signature Wallet 👛
                    </>
                  }
                  type="info"
                />
