...
export default function CreateMultiSigModal({
  ...
  const handleSubmit = () => {
    try {
      ...
      tx(
        ...
        setPendingCreate(false);
        console.log("CREATE - MULTI SIGNATURE WALLET SUBMIT FAILED - 1: ", err);
        throw err;
      });

      setTxSent(true);
    } catch (e) {
      console.log("CREATE - MULTI SIGNATURE WALLET SUBMIT FAILED: ", e);
    }
  };
  ...
  return (
    <>
      ...
      <Modal
        title="Create - Multi Signature Wallet"
      ...
      >
        {txSent && (
          <CreateModalSentOverlay
            txError={txError}
            txSuccess={txSuccess}
            pendingText="Creating Multi Signature Wallet"
            successText="Multi Signature Wallet Created"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div style={{ width: "90%" }}>
            <EtherInput
              placeholder="Fund your Multi Signature Wallet (optional)"
              price={price}
              mode="USD"
              value={amount}
              onChange={setAmount}
